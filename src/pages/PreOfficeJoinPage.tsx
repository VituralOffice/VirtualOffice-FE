...
import MicOptionsPopups from '../components/popups/MicOptionsPopup'
import VideoOptionsPopups from '../components/popups/VideoOptionsPopup'
...
const OptionButton = styled.div<ButtonProps>`
  ...
  .line {
    ...
    width: 2px;
    height: 20px;
    background-color: ${(props) => props.isEnabled ? 'rgb(6, 214, 160)' : 'rgba(255, 48, 73, 0.2)'};
  }
`
...
function CustomToggleButton({ onToggle, OnIcon, OffIcon, MenuPopup }) {
  ...
  return (
    <>
      <OptionButton isEnabled={isButtonEnabled} onClick={handleToggle}>
        ...
        <OptionMenuToggleButton isActive={isButtonEnabled} isEnabled={isMenuEnabled} onClick={handleToggleOpenMenu}>
          <span>
            <KeyboardArrowDownRoundedIcon />
          </span>
        </OptionMenuToggleButton>
      </OptionButton>
      {isMenuEnabled && MenuPopup}
    </>
  )
}
